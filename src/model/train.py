...
to predict TB incidence per 100k from vaccination, economic, HIV, and health
covariates. Uses a temporal split: train 2000-2017, test 2018-2022.
"""
...
from src.model.features import FEATURE_COLUMNS, build_feature_matrix
...
def main():
    ...
    feature_importance = {
        col: float(imp) for col, imp in zip(FEATURE_COLUMNS, rf.feature_importances_)
    }
    ...
    for col, imp in sorted(feature_importance.items(), key=lambda x: -x[1])[:6]:
